...
void create_chunk_pkts(int type, short pkt_len, u_int seq, u_int ack, 
						uint8_t *data, char* dst) {
...
ssize_t packet_sender(bt_config_t *config, data_packet_t* pkt, bt_peer_t *peer, int sock) {
    ...
    int flag;
    hostToNet(pkt);
    get = spiffy_sendto(config->sock, pkt, pkt_size, 0, 
...
int init_mapping_per_get_req(char* chunkFile, char* output_file) {
    ...
    while(fgets(line, MAX_LINE_SIZE, file) != NULL)
    {
        ...
        mapping_per_get_req.chunks[j].downloaded = 0;
        j++;
    }
...
int whohas_req(char *chunkfile, int sock, bt_config_t *config) {
    ...
    printf("Whohas req:%s", chunkfile);
    FILE*   fp_chunks;
    char    line[MAX_LINE_SIZE];
    unsigned char    whohas_packet[MAX_PACKET_SIZE];
    int     i, offset, k, j, nchunks = 0;
    char*   ptr, ptr_hash;
    ssize_t whohas;
...
void data_packet_handler(bt_config_t *config1, char* buf, bt_peer_t *peer, int sock) {  
    ...
    int i,k, hash_matched, offset, index = 0;;
	FILE*   fp_chunks;
	char    line[MAX_LINE_SIZE];
	char    temp_hash[CHUNK_HSIZE];
	char* ptr_hash;
